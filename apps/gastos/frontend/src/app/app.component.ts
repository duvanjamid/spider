...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [NgFor, NgIf, FormsModule, ButtonModule, CardModule, ChartModule, TagModule],
  styles: [`
    .wrap { max-width: 1120px; margin: 0 auto; padding: 0 16px 90px; }

    /* App bar sobrio (estilo Waze): superficie limpia, borde fino. */
    .bar { position: sticky; top: 0; z-index: 20; display: flex; align-items: center; gap: 10px;
           padding: 12px 12px; margin: 0 -16px 8px; padding-left: 16px; padding-right: 16px;
           background: color-mix(in srgb, var(--panel) 88%, transparent);
           backdrop-filter: blur(10px); border-bottom: 1px solid var(--border); }
    .brand { display: flex; align-items: center; gap: 9px; font-weight: 800; font-size: 1.2rem; }
    .brand .blogo { width: 27px; height: 27px; color: var(--accent-strong); }
    .brand .wm { color: var(--fg); }
    .topbar-back { width: 38px; height: 38px; border-radius: 11px; border: 1px solid var(--border);
                   background: var(--panel); color: var(--fg); display: grid; place-items: center; cursor: pointer;
                   font-size: 1rem; transition: transform .06s; }
    .topbar-back:active { transform: scale(.92); }
    .view-title { font-weight: 800; font-size: 1.15rem; }
    .env { font-size: .66rem; font-weight: 800; letter-spacing: .5px; padding: 3px 7px; border-radius: 6px;
           background: #d98a1f; color: #201400; text-transform: uppercase; }
    .spacer { flex: 1; }
    .month { display: flex; align-items: center; gap: 4px; }
    .month b { min-width: 128px; text-align: center; text-transform: capitalize; font-size: .95rem; color: var(--fg); }
    .mnav { width: 34px; height: 34px; border-radius: 10px; border: 1px solid var(--border);
            background: var(--panel); color: var(--accent-strong); cursor: pointer; display: grid; place-items: center;
            font-size: .85rem; transition: transform .06s; }
    ...
    .up { color: #e0574f; } .down { color: var(--accent-strong); } .accent { color: var(--accent-strong); }
    ...
    .det-amt { display: flex; align-items: center; gap: 10px; font-size: 1.5rem; font-weight: 800; padding: 2px 0 12px; }
    .det-amt .dot { width: 14px; height: 14px; border-radius: 50%; }
    .det-amt .muted { font-size: .9rem; font-weight: 500; margin-left: auto; }
    .det-row { display: flex; justify-content: space-between; gap: 12px; padding: 10px 0; border-top: 1px solid var(--border); }
    ...
    /* Campos */
    .inp, .sel, .ta { width: 100%; padding: 11px 12px; border-radius: 11px; border: 1px solid var(--border);
           background: var(--panel-2); color: var(--fg); font-size: 1rem; font-family: inherit; }
    .inp:focus, .sel:focus, .ta:focus { outline: none; border-color: var(--accent); box-shadow: 0 0 0 3px var(--accent-weak); }
    .ta { min-height: 130px; resize: vertical; }
    .field { display: flex; flex-direction: column; gap: 6px; }
    .field label { font-size: .8rem; color: var(--muted); }
    .field.full { grid-column: 1 / -1; }
    .form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
    .hint { font-size: .82rem; color: var(--accent-strong); }
    .actionbar { display: flex; gap: 10px; justify-content: flex-end; margin-top: 18px; flex-wrap: wrap; }
    ...
    .chip.sel { border-color: var(--accent); background: var(--accent-weak); }
    .chip small { color: var(--muted); }

    /* Filas simples (recurrentes / conexiones / ingresos) */
    .cat-add { display: flex; gap: 8px; align-items: center; margin-bottom: 14px; flex-wrap: wrap; }
    .cat-row { display: flex; align-items: center; gap: 10px; padding: 10px 4px; border-bottom: 1px solid var(--border); }

    /* Editor de categoría */
    .card { border: 1px solid var(--border); border-radius: 16px; padding: 16px; background: var(--panel); box-shadow: var(--shadow); }
    .cat-editor { border: 1px solid var(--accent-line); border-radius: 16px; padding: 16px;
    ...
    .ce-preview { width: 56px; height: 56px; border-radius: 15px; display: grid; place-items: center; color: #fff;
                  font-size: 1.45rem; flex: none; box-shadow: 0 6px 16px rgba(24,32,20,.22); }
    .ce-name { flex: 1; }
    ...
    .ico-pick.on { border-color: var(--accent); background: var(--accent-weak); color: var(--accent-strong); }
    ...
    .ce-actions { display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px; }

    /* Grid de categorías (tarjetas) */
    .cat-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    @media (min-width: 560px) { .cat-grid { grid-template-columns: 1fr 1fr 1fr; } }
    ...
    .cat-card .cc-bud b { color: var(--accent-strong); font-weight: 800; }
    ...
    .cat-card .cc-btn.shared { color: var(--accent-strong); }
    .cat-card .cc-btn.del:hover { color: #e0574f; }
    ...
    .card-title { font-size: 1.02rem; font-weight: 800; color: var(--fg); }

    /* Donut central con labels DEBAJO */
    .donut-hero { display: flex; flex-direction: column; align-items: center; gap: 14px; }
    .donut-ring { position: relative; width: 200px; max-width: 60vw; aspect-ratio: 1/1; }
    ...
    .inc-row { display: flex; align-items: center; gap: 10px; padding: 10px 4px; border-bottom: 1px solid var(--border); }

    /* Gastos hormiga */
    .ant-head { display: flex; align-items: baseline; gap: 10px; flex-wrap: wrap; margin-bottom: 8px; }
    .ant-head .big { font-size: 1.5rem; font-weight: 800; color: #d98a1f; letter-spacing: -.5px; }
    ...
    .ant-item .ac { font-size: .74rem; font-weight: 800; padding: 2px 8px; border-radius: 999px;
                    background: color-mix(in srgb, #d98a1f 18%, transparent); color: #d98a1f; white-space: nowrap; }
    .ant-item .at { font-weight: 700; white-space: nowrap; }

    /* Animación de análisis IA */
    .scan-anim { display: flex; flex-direction: column; align-items: center; gap: 18px; padding: 26px 8px 8px; }
    ...
    .scan-status { text-align: center; } .scan-status .t { font-weight: 700; }
    .scan-status .s { color: var(--muted); font-size: .88rem; margin-top: 3px; min-height: 20px; }
    ...
    .region .rlbl { position: absolute; top: -20px; left: -2px; font-size: .66rem; font-weight: 700;
              background: var(--accent); color: var(--on-accent); padding: 1px 6px; border-radius: 5px; white-space: nowrap; }
    ...
    .tcard.on { border-color: var(--accent); background: var(--accent-soft); }
    ...
    .tcard .ck { margin-left: auto; color: var(--accent-strong); }
    ...
    /* Navegación inferior con FAB central */
    ...
    .bnav-fab { justify-self: center; width: 58px; height: 58px; margin-top: -26px; border-radius: 50%;
                border: 4px solid var(--bg); background: var(--accent); color: var(--on-accent); font-size: 1.4rem;
    ...
    /* Elección: tomar foto / subir imagen */
    .picker { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; padding: 4px 0 8px; }
    .picker button { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 24px 12px;
                     border: 1px solid var(--border); border-radius: 16px; background: var(--panel);
                     color: var(--fg); font: inherit; cursor: pointer; transition: border-color .15s, transform .06s; box-shadow: var(--shadow); }
    .picker button:hover { border-color: var(--accent); }
    .picker button:active { transform: scale(.97); }
    .picker button i { font-size: 1.8rem; color: var(--accent-strong); }
    ...
    .profile { border-radius: 18px; overflow: hidden; border: 1px solid var(--border); background: var(--panel); box-shadow: var(--shadow); }
    .pf-cover { height: 74px; background: var(--accent); }
    ...
    .pf-avatar { width: 80px; height: 80px; border-radius: 50%; margin: 0 auto 10px; background: var(--panel-2);
                 border: 3px solid var(--panel); box-shadow: 0 4px 16px rgba(0,0,0,.18); overflow: hidden;
    ...
    .pf-chip.guest { background: color-mix(in srgb, #d98a1f 16%, transparent); color: #d98a1f; }
    .acc-group { margin-top: 18px; }
    .acc-title { font-size: .72rem; font-weight: 800; letter-spacing: .6px; text-transform: uppercase; color: var(--muted); margin: 0 4px 8px; }
    ...
    .logout { width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px; padding: 14px;
              border: 1px solid color-mix(in srgb, #e0574f 40%, var(--border)); border-radius: 14px; background: none;
              color: #e0574f; font: inherit; font-weight: 700; font-size: .96rem; cursor: pointer; }
    .logout:active { background: color-mix(in srgb, #e0574f 10%, transparent); }
    .acc-cta { margin: 14px 0 2px; }
    .acc-foot { text-align: center; color: var(--muted); font-size: .76rem; margin: 18px 0 2px; }
    .link-btn { background: none; border: none; color: var(--accent-strong); font: inherit; font-size: .74rem; font-weight: 700; cursor: pointer; padding: 0; }
    ...
    .bnav-badge { position: absolute; top: -6px; right: -10px; min-width: 16px; height: 16px; padding: 0 4px;
                  border-radius: 999px; background: #e0574f; color: #fff; font-size: .64rem; font-weight: 800;
  ...
  template: `
    ...
    <div class="wrap" *ngIf="!onboarding()">
      <!-- Barra: en pestaña muestra logo + mes; en vista muestra atrás + título. -->
      <div class="bar">
        <ng-container *ngIf="!view(); else viewBar">
          <div class="brand">
          ...
          <div class="month" *ngIf="tab() === 0 || tab() === 3">
        ...
        <ng-template #viewBar>
          <button class="topbar-back" (click)="goBack()" aria-label="Atrás"><i class="fa-solid fa-arrow-left"></i></button>
      ...
      <div class="muted" *ngIf="!aiEnabled() && !view()" style="margin:10px 0 0"><i class="pi pi-info-circle"></i> Configura GEMINI_API_KEY para el escaneo con IA.</div>
      ...
      <section class="page" *ngIf="!view() && tab() === 0">
        ...
        <div style="margin-bottom:16px" *ngIf="summary() as s">
          <p-card>
            ...
            <div class="tope" *ngIf="s.income > 0; else noIncome">
              <div class="pbar">
                <div class="fill" [style.width.%]="topePct()" [style.background]="topeOver() ? '#e0574f' : 'var(--accent)'"></div>
              </div>
              <div class="fig"><b [class.up]="topeOver()">{{ fmt(s.total) }}</b><span class="muted"> / {{ fmt(s.income) }}</span></div>
              <div class="muted" style="width:100%;font-size:.84rem" [class.up]="topeOver()">
                {{ topeOver() ? ('Te pasaste ' + fmt(s.total - s.income)) : ('Te quedan ' + fmt(s.income - s.total)) }} · {{ topePct() }}% de tus ingresos
              </div>
            </div>
          ...
          <p-card header="Presupuesto del mes" *ngIf="budgeted().length; else noBudget">
            <div class="bud-summary">
              <div style="width:132px;height:132px;position:relative">
                ...
                <div class="donut-center"><b [class.up]="budgetTotals().remaining < 0">{{ budgetTotals().pct }}%</b><small>usado</small></div>
              </div>
              <div class="bud-legend">
                <div><span class="sw" [style.background]="chartAccent()"></span> Gastado <b>{{ fmt(budgetTotals().spent) }}</b></div>
                <div><span class="sw" [style.background]="dark() ? '#2a2f24' : '#e2e6dd'"></span>
            ...
            <div class="bud2" *ngFor="let b of budgeted()">
              ...
              <div class="pbar"><div class="fill" [style.width.%]="b.pct" [style.background]="b.over ? '#e0574f' : b.color"></div></div>
        ...
        <ng-template #plainCatBar>
          <div style="margin-top:16px">
            <p-card header="Gasto por categoría (mes)">
              <div class="chart-box" style="height:220px" *ngIf="(summary()?.byCategory?.length || 0) > 0; else noData">
        ...
        <div style="margin-top:16px">
          <p-card [header]="(summary()?.income || 0) > 0 ? 'Quema del presupuesto' : 'Cómo va el mes'">
            <div class="chart-box" style="height:230px" *ngIf="(summary()?.count || 0) > 0; else noData">
        ...
        <div style="margin-top:16px" *ngIf="ant() as a">
          <p-card *ngIf="a.groups.length" header="Gastos hormiga">
            ...
            <div class="ant-item" *ngFor="let g of a.groups">
              <span class="an">{{ g.label }}</span><span class="ac">×{{ g.count }}</span><span class="at">{{ fmt(g.total) }}</span>
            </div>
            <p class="muted" style="margin:10px 0 0;font-size:.82rem">
              <i class="fa-solid fa-lightbulb" style="color:#d98a1f"></i>
      ...
      <!-- ═══ Pestaña: Movimientos ═══ -->
      <section class="page" *ngIf="!view() && tab() === 3">
        <div class="filters">
      ...
      <!-- ═══ Pestaña: Mi cuenta ═══ -->
      <section class="page" *ngIf="!view() && tab() === 5">
        <div class="account">
          <div class="profile">
          ...
          <div class="acc-cta" *ngIf="isGuest()">
            <a href="/admin/" style="text-decoration:none"><p-button label="Entrar con Google" icon="fa-brands fa-google" [style]="{ width: '100%' }" /></a>
            <p class="muted" style="text-align:center;font-size:.82rem;margin:8px 4px 0">Inicia sesión para guardar tus gastos y compartir con tu hogar.</p>
          </div>
          ...
          <div class="acc-group">
            ...
            <div class="menu">
              <button (click)="openHome()"><i class="fa-solid fa-house-user"></i><span>Hogar (compartir)</span>
                <span class="badge" *ngIf="conns()?.incoming?.length">{{ conns()?.incoming?.length }}</span><i class="fa-solid fa-chevron-right go"></i></button>
              <button (click)="openCats()"><i class="fa-solid fa-tags"></i><span>Mis categorías</span><i class="fa-solid fa-chevron-right go"></i></button>
              ...
              <button (click)="openRecurring()"><i class="fa-solid fa-rotate"></i><span>Gastos recurrentes</span><i class="fa-solid fa-chevron-right go"></i></button>
              <button (click)="openCompare()"><i class="fa-solid fa-code-compare"></i><span>Comparar meses</span><i class="fa-solid fa-chevron-right go"></i></button>
              <button (click)="exportCsv()"><i class="fa-solid fa-file-csv"></i><span>Exportar CSV</span><i class="fa-solid fa-chevron-right go"></i></button>
            </div>
          </div>
          ...
          <div class="acc-group" *ngIf="!isGuest()">
            ...
            <div class="toggle-row">
              <i class="fa-solid fa-bell" style="color:var(--accent-strong);font-size:1.1rem"></i>
              <div class="tr-body"><b>Notificaciones en este dispositivo</b><small>{{ pushHint() }}</small></div>
              <button class="sw-toggle" [class.on]="pushOn()" [disabled]="pushBusy() || !pushSupported()" (click)="togglePush()" [attr.aria-pressed]="pushOn()"></button>
            </div>
          </div>
      ...
      <section class="page" *ngIf="view() === 'register'">
        ...
        <div *ngIf="regStep() === 'choose'">
          ...
          <div class="picker">
            ...
            <button (click)="takePhoto()" [disabled]="!aiEnabled()"><i class="fa-solid fa-camera"></i><b>Tomar foto</b><small>Cámara</small></button>
            <button (click)="uploadImage()" [disabled]="!aiEnabled()"><i class="fa-solid fa-image"></i><b>Subir imagen</b><small>Galería · hasta 3</small></button>
          </div>
        </div>

        <!-- Pegar texto -->
        <div *ngIf="regStep() === 'text'">
          ...
          <div class="actionbar">
            <p-button label="Atrás" [text]="true" (onClick)="regStep.set('choose')" />
            <p-button label="Analizar" icon="pi pi-sparkles" (onClick)="runTextScan()" [disabled]="!pastedText.trim()" />
          </div>
        </div>

        <!-- Escaneo / formulario -->
        <div *ngIf="regStep() === 'sheet'">
          <div class="scan-anim" *ngIf="sheetState() === 'loading'">
            <div class="doc"><div class="laser"></div><div class="ln l"></div><div class="ln m"></div><div class="ln s"></div><div class="ln m"></div><div class="ln l"></div><div class="ln s"></div></div>
            <div class="scan-status"><div class="t">Analizando con IA<span *ngIf="imageCount() > 1"> · {{ imageCount() }} fotos</span></div><div class="s">{{ scanPhase() }}</div></div>
            <div class="beads"><i></i><i></i><i></i></div>
          </div>

          <div class="scan-anim" *ngIf="sheetState() === 'error'">
            <i class="pi pi-exclamation-triangle" style="font-size:2.4rem;color:#e0574f"></i>
          ...
          <div class="scan-anim" *ngIf="sheetState() === 'unreadable'">
            <i class="pi pi-image" style="font-size:2.4rem;color:#d98a1f"></i>
          ...
          <div *ngIf="sheetState() === 'form'">
            ...
            <div *ngIf="sheetTab() === 'datos'">
              <div class="form-grid">
                <div class="field full" *ngIf="montos().length">
                  ...
                  <div class="montos">
                    <div class="chip" *ngFor="let m of montos()" [class.sel]="form.amount === m.valor" (click)="form.amount = m.valor"><b>{{ fmt(m.valor) }}</b> <small>{{ m.etiqueta }}</small></div>
                  </div>
                </div>
            ...
            <div *ngIf="sheetTab() === 'imagen' && hasAnalyzedImage()">
              ...
              <div class="analyzed">
                ...
                <div class="region" *ngFor="let r of regiones()" [style.top.%]="r.box[0]/10" [style.left.%]="r.box[1]/10" [style.height.%]="(r.box[2]-r.box[0])/10" [style.width.%]="(r.box[3]-r.box[1])/10">
            ...
            <div class="actionbar">
              ...
              <p-button label="Guardar" icon="pi pi-check" (onClick)="save()" [loading]="saving()" [disabled]="!form.amount || form.amount <= 0" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="page" *ngIf="view() === 'detail'">
        <div *ngIf="detail() as d">
          <div class="card" *ngIf="!editing()">
            <div class="det-amt">
              <span class="dot" [style.background]="d.categoryColor || '#9aa3b2'"></span>{{ fmt(d.amount, d.currency) }}
          ...
          <div class="actionbar" *ngIf="!editing()">
      ...
      <section class="page" *ngIf="view() === 'category-edit'">
        <div class="cat-editor">
          ...
          <div class="ico-row">
            <button type="button" class="ico-pick" *ngFor="let ic of catIcons" [class.on]="catForm.icon === ic" (click)="catForm.icon = ic"><i [class]="ic"></i></button>
          </div>
          ...
          <div class="col-row">
            ...
            <label class="col-pick custom" title="Color personalizado"><i class="fa-solid fa-eye-dropper"></i><input type="color" [(ngModel)]="catForm.color" /></label>
          </div>
      ...
      <section class="page" *ngIf="view() === 'income'">
        ...
        <div class="card">
          ...
          <div class="cat-add" style="margin-bottom:0">
            ...
            <p-button label="Añadir" icon="pi pi-plus" (onClick)="addIncome()" [disabled]="!incForm.amount || incForm.amount <= 0" />
          </div>
        </div>
        <div style="margin-top:14px">
          ...
          <div style="display:flex;justify-content:space-between;margin-top:12px;font-weight:800" *ngIf="incomeList().length"><span>Tope del mes</span><span>{{ fmt(incomeTotal()) }}</span></div>
      ...
      <section class="page" *ngIf="view() === 'recurring'">
        ...
        <div class="actionbar"><p-button label="Aplicar al mes actual" icon="pi pi-check" (onClick)="applyRecurring()" [loading]="applying()" /></div>
      </section>

      <!-- ═══ Vista: Comparar meses ═══ -->
      <section class="page" *ngIf="view() === 'compare'">
        <div class="card">
          <div class="cat-add" style="margin-bottom:0">
        ...
        <div class="cmp" style="margin-top:14px" *ngIf="cmpSummA() && cmpSummB()">
        ...
        <div class="chart-box" style="height:230px;margin-top:14px" *ngIf="cmpSummA() && cmpSummB()">
          <p-chart type="bar" [data]="cmpData()" [options]="barOptionsV()" />
        </div>
      </section>

      <!-- ═══ Vista: Hogar ═══ -->
      <section class="page" *ngIf="view() === 'home'">
        ...
        <ng-container *ngIf="conns() as cn">
          ...
          <div *ngIf="cn.accepted.length">
            ...
            <div class="cat-row" *ngFor="let c of cn.accepted">
              <i class="fa-solid fa-circle-check" style="color:var(--accent-strong)"></i>
              <span style="flex:1">{{ c.email }}</span>
          ...
          <p class="muted" *ngIf="!cn.incoming.length && !cn.accepted.length && !cn.outgoing.length" style="text-align:center;padding:16px 0">Aún no tienes conexiones. Invita a alguien por su correo.</p>
        </ng-container>
      </section>

      <!-- ═══ Navegación inferior ═══ -->
      <nav class="bnav">
        <button class="bnav-item" [class.on]="!view() && tab() === 0" (click)="goTab(0)"><i class="fa-solid fa-gauge-high"></i><span>Estatus</span></button>
        <button class="bnav-item" [class.on]="!view() && tab() === 3" (click)="goTab(3)"><i class="fa-solid fa-list-ul"></i><span>Movimientos</span></button>
        <button class="bnav-fab" (click)="openRegister()" aria-label="Registrar gasto"><i class="fa-solid fa-plus"></i></button>
        <button class="bnav-item" [class.on]="!view() && tab() === 4" (click)="openPrices()"><i class="fa-solid fa-tags"></i><span>Precios</span></button>
        <button class="bnav-item" [class.on]="!view() && tab() === 5" (click)="goTab(5)" aria-label="Mi cuenta">
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  // Pestañas (0=Estatus, 3=Movimientos, 4=Precios, 5=Cuenta) y vista de página completa.
  ...
  catForm: { id: number | null; name: string; color: string; icon: string; amount: number | null } =
    { id: null, name: '', color: '#8db63c', icon: 'fa-solid fa-wallet', amount: null };
  readonly catIcons = [
  ...
  readonly catColors = ['#8db63c', '#4a9e5c', '#0ea5a3', '#3b82f6', '#8b5cf6', '#ec4899',
    '#ef4444', '#f59e0b', '#d98a1f', '#a3a635', '#64748b', '#78716c'];
  ...
  readonly chartAccent = computed(() => (this.dark() ? '#a6cc4e' : '#8db63c'));

  // ── Colores de gráficos según tema ──
  private tick(): string { return this.dark() ? '#9aa392' : '#5b6656'; }
  private gridc(): string { return this.dark() ? 'rgba(255,255,255,.07)' : 'rgba(24,32,20,.08)'; }
  private legend(): string { return this.dark() ? '#e7ebe0' : '#1e261c'; }
  ...
  readonly budgetDonut = computed(() => {
    ...
    return { labels: ['Gastado', 'Restante'],
      datasets: [{ data: [over ? t.budget : spent, rest],
        backgroundColor: [over ? '#e0574f' : this.chartAccent(), this.dark() ? '#2a2f24' : '#e2e6dd'], borderWidth: 0 }] };
  });
  readonly budgetDonutOpts = computed(() => ({ maintainAspectRatio: false, cutout: '72%', plugins: { legend: { display: false } } }));
  ...
  // Presupuesto vs gastado por categoría: la barra «Gastado» usa el color de la categoría.
    ...
    return { labels: bc.map((c) => c.name),
      datasets: [
        { label: 'Gastado', data: bc.map((c) => c.total),
          backgroundColor: bc.map((c) => (c.total > c.budget ? '#e0574f' : c.color)), borderRadius: 6 },
        { label: 'Presupuesto', data: bc.map((c) => c.budget),
          backgroundColor: this.dark() ? '#2a2f24' : '#e2e6dd', borderRadius: 6 },
      ] };
  });
  ...
  readonly burnData = computed(() => {
    ...
    for (let d = 1; d <= days; d++) { const v = cum[d - 1]; if (v != null) last = v; spent.push(d <= elapsed ? last : null); }
    const labels = Array.from({ length: days }, (_, i) => String(i + 1));
    const acc = this.chartAccent();
    const datasets: Record<string, unknown>[] = [
      { label: 'Gasto acumulado', data: spent, borderColor: acc, backgroundColor: 'transparent', tension: 0.3, pointRadius: 0, fill: false },
    ];
    const tope = s?.income ?? 0;
    if (tope > 0) {
      const remaining = spent.map((v) => (v == null ? null : Math.max(tope - v, 0)));
      datasets.push({ label: 'Presupuesto restante', data: remaining, borderColor: '#e0574f', backgroundColor: 'rgba(224,87,79,.10)', tension: 0.3, pointRadius: 0, fill: true });
      datasets.push({ label: 'Tope', data: new Array(days).fill(tope), borderColor: this.dark() ? '#4b5343' : '#c8cebb', borderDash: [4, 4], pointRadius: 0, fill: false });
    }
  ...
  readonly cmpData = computed(() => ({
    ...
    datasets: [{ data: [this.cmpSummA()?.total ?? 0, this.cmpSummB()?.total ?? 0], backgroundColor: [this.chartAccent(), '#3b82f6'], borderRadius: 8 }],
  }));
  ...
  readonly pieData = computed(() => {
    ...
    return { labels: bc.map((c) => c.name),
      datasets: [{ data: bc.map((c) => c.total), backgroundColor: bc.map((c) => c.color),
        borderColor: this.dark() ? '#191d15' : '#ffffff', borderWidth: 2, hoverOffset: 8, spacing: 2 }] };
  });
  ...
  // ── Navegación de vistas (nativa, con barra atrás + gesto atrás) ──
  private nav(v: View): void { this.view.set(v); this.pushGuard(); }
  goBack(): void {
    if (this.view() === 'register') {
      if (this.regStep() !== 'choose') { this.regStep.set('choose'); return; }
  ...
  // Gesto/botón atrás: retrocede en la vista, luego a Estatus; nunca cierra la PWA.
  private onPopState = (): void => { this.goBack(); this.pushGuard(); };

  /** Cambia de pestaña inferior y cierra cualquier vista abierta. */
  goTab(n: number): void {
    this.view.set('');
    this.tab.set(n);
    if (n === 5) this.openNotifs();
  }
  ...
  notifTint(kind: string): string {
    return kind === 'connection_invite' ? '#3b82f6'
      : kind === 'connection_accepted' ? '#4a9e5c'
      : kind === 'category_shared' ? '#d98a1f'
      : kind === 'budget_exceeded' ? '#e0574f'
      : '#8b5cf6';
  }
  ...
  finishOnboarding(): void {
    ...
    this.api.onboarding(this.chosen()).subscribe({
      next: () => { this.saving.set(false); this.onboarding.set(false); this.loadCategories(); this.loadBudgets(); this.reload(); },
  ...
  openRegister(): void {
    ...
    this.suggested.set(null); this.scanItems.set([]); this.form = this.emptyForm();
    this.pastedText = ''; this.regStep.set('choose'); this.sheetState.set('form'); this.nav('register');
  }
  chooseManual(): void {
    this.scanned = false; this.montos.set([]); this.regiones.set([]); this.lastImageUrl.set(null);
    this.suggested.set(null); this.scanItems.set([]); this.form = this.emptyForm();
    this.sheetTab.set('datos'); this.sheetState.set('form'); this.regStep.set('sheet');
  }
  chooseText(): void { this.pastedText = ''; this.regStep.set('text'); }
  toManual(): void { this.scanned = false; this.montos.set([]); this.sheetTab.set('datos'); this.sheetState.set('form'); this.regStep.set('sheet'); }
  takePhoto(): void { this.cameraInput?.nativeElement.click(); }
  uploadImage(): void { this.fileInput?.nativeElement.click(); }
  pickAgain(): void { this.regStep.set('choose'); }
  ...
  onFile(ev: Event): void {
    ...
    if (this.view() !== 'register') this.nav('register');
    this.regStep.set('sheet'); this.sheetState.set('loading'); this.startPhases();
  ...
  private startPhases(): void {
    const phases = ['Preparando imagen…', 'Detectando establecimiento…', 'Leyendo montos…', 'Buscando NIT y fecha…', 'Clasificando la categoría…'];
  ...
  private runScan(): void {
    ...
    this.regStep.set('sheet'); this.sheetState.set('loading');
  ...
  private randColor(): string {
    const pal = ['#8db63c', '#4a9e5c', '#0ea5a3', '#3b82f6', '#a855f7', '#ec4899', '#14b8a6', '#f59e0b'];
  ...
  sendInvite(): void {
    ...
    if (!e) return;
    this.api.invite(e).subscribe({ next: () => { this.inviteEmail = ''; this.reloadConns(); }, error: () => alert('No se pudo invitar (revisa el correo).') });
  }
  ...
  openPrices(): void { this.view.set(''); this.tab.set(4); if (!this.pricesLoaded()) this.loadPrices(); }
  loadPrices(): void {
