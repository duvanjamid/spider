package com.spider.alertas.session;
...
/**
 * Identifica al usuario por la cookie {@code spider_session} del admin (HMAC con
 * {@code AUTH_JWT_SECRET}). Sin sesión válida devuelve un invitado. El email real
 * NUNCA se expone a la comunidad: la UI usa el seudónimo del {@code reporter}.
 */
public final class Identity {
    ...
    private String verify(String token) {
        ...
        String p = token.substring(0, dot), sig = token.substring(dot + 1);
